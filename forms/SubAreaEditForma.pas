...
begin
  ...
  if dblArea.Text = '' then
  begin
    CnErrors.SetError(dblArea, rsSelectCity, iaTopCenter, bsNeverBlink);
    En := False;
  end
  else
    CnErrors.Dispose(dblArea);

  OkCancelFrame.bbOk.Enabled := En;
end;

procedure TSubAreaViewForm.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TSubAreaViewForm.dblAreaEditButtons0Click(Sender: TObject; var Handled: Boolean);
var
  i: Integer;
begin
  Handled := true;
