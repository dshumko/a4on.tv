...
uses
  ...
  AtrPages, frxClass, frxPreview, FIBDataSet, pFIBDataSet, PrjConst, FIBDatabase, pFIBDatabase;
...
procedure TapgCustomerCard.srcForScroolWaitDataChange(Sender: TObject; Field: TField);
begin

  if (not fActiveForm) or (not frxPreview.Visible) or (srcForScroolWait.DataSet.FieldByName('CUSTOMER_ID').IsNull) then
    Exit;
