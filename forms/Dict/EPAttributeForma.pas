...
begin
  ...
  with TEPAttributForm.Create(Application) do
    try
...
procedure TEPAttributForm.dbluAttributeChange(Sender: TObject);
begin
  cbbList.Items.Clear;
...
procedure TEPAttributForm.FormKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Shift = [ssCtrl]) and (Ord(Key) = VK_RETURN) then
    OkCancelFrame1bbOkClick(Sender);
end;

procedure TEPAttributForm.OkCancelFrame1bbOkClick(Sender: TObject);
var
  errors: Boolean;
  s: string;
  reg: string;
