...
type
  TNodeLayoutsForm = class(TGridForm)
...
var
  NodeLayoutsForm: TNodeLayoutsForm;
...
procedure TNodeLayoutsForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  dsLayout.Close;
  dsFileType.Close;
...
procedure TNodeLayoutsForm.FormCreate(Sender: TObject);
begin
  inherited;
  fNodeLayout := (dmMain.GetSettingsValue('NODE_LAYOUT') = '1');
end;

procedure TNodeLayoutsForm.actNewExecute(Sender: TObject);
begin
  inherited;
  if fCanEdit then
    StartEdit(True);
...
procedure TNodeLayoutsForm.btnCancelLinkClick(Sender: TObject);
begin
  inherited;
  pnlBottom.Enabled := True;
end;

procedure TNodeLayoutsForm.btnSaveLinkClick(Sender: TObject);
var
  errors: Boolean;
begin
  errors := False;
...
procedure TNodeLayoutsForm.dbGridGetCellParams(Sender: TObject; Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  inherited;
...
procedure TNodeLayoutsForm.actAddLExecute(Sender: TObject);
var
  NL_ID: Integer;
begin
  inherited;
...
procedure TNodeLayoutsForm.actEditLExecute(Sender: TObject);
var
  NL_ID: Integer;
begin
  inherited;
...
procedure TNodeLayoutsForm.actCopyExecute(Sender: TObject);
var
  old_n: string;
begin
  inherited;
...
procedure TNodeLayoutsForm.actDeleteExecute(Sender: TObject);
begin
  inherited;
...
procedure TNodeLayoutsForm.actDelLExecute(Sender: TObject);
var
  s: string;
begin
  inherited;
...
procedure TNodeLayoutsForm.actEditExecute(Sender: TObject);
begin
  inherited;
  if fCanEdit then
    StartEdit();
...
procedure TNodeLayoutsForm.FormShow(Sender: TObject);
var
  vFull: Boolean;
  i, j: Integer;
begin
  ...
  for i := 0 to dbgLayout.Columns.Count - 1 do
  begin
    if dbgLayout.Columns[i].FieldName = 'CUST_QNT' then
    begin
      dbgLayout.Columns[i].KeyList.Clear;
      dbgLayout.Columns[i].PickList.Clear;
      dbgLayout.Columns[i].KeyList.Add('-1');
      dbgLayout.Columns[i].PickList.Add('Любое');
      for j := 0 to 999 do
      begin
        dbgLayout.Columns[i].KeyList.Add(j.ToString);
        dbgLayout.Columns[i].PickList.Add(j.ToString);
      end
    end;
  end;
end;

procedure TNodeLayoutsForm.srcDataSourceDataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
procedure TNodeLayoutsForm.srcLayoutDataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
procedure TNodeLayoutsForm.StartEdit(const New: Boolean = False);
begin
  inherited;
end;

procedure TNodeLayoutsForm.UpdateBottom;
begin
