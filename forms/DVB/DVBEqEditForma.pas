...
begin
  ...
  with TDVBEqEditForm.Create(Application) do
    try
      dsMan.Open;
      dsType.Open;
      if dmMain.AllowedAction(rght_Dictionary_DigitEQP) then
      begin
        edtNumber.Text := EQ_N;
        with TpFIBQuery.Create(Nil) do
          try
            ...
            ExecQuery;
            if not FieldByName('EQ_ID').IsNull then
              id := FieldByName('EQ_ID').AsInteger;
            if not FieldByName('EQ_STATE').IsNull then
              i := FieldByName('EQ_STATE').AsInteger
            else
              i := 1;
            cbState.Value := i;
            if not FieldByName('notice').IsNull then
              s := FieldByName('notice').AsString
            else
              s := '';
            mmoNotice.Lines.Text := s;
            if not FieldByName('EQ_MAN').IsNull then
              cbMAN.Value := FieldByName('EQ_MAN').AsInteger;
            if not FieldByName('EQ_TYPE').IsNull then
              lcbType.Value := FieldByName('EQ_TYPE').AsInteger;
            if not FieldByName('TEXT_ENCODE').IsNull then
              cbEncode.Value := FieldByName('TEXT_ENCODE').AsString;
            Close;
            Transaction.Commit;
          finally
            Free;
          ...
          try
            with TpFIBQuery.Create(dmMain) do
              try
                ...
                SQL.Text :=
                  'execute procedure EQUIPMENT_UPD(:EQ_ID, :EQ_N, :EQ_TYPE, :EQ_STATE, :EQ_MAN, :NOTICE, :TEXT_ENCODE)';
                ParamByName('EQ_ID').AsInteger := id;
                if VarIsNumeric(cbMAN.Value) then
                  ParamByName('EQ_MAN').AsInteger := cbMAN.Value;
                if VarIsNumeric(lcbType.Value) then
                  ParamByName('EQ_TYPE').AsInteger := lcbType.Value;
                ParamByName('EQ_N').AsString := EQ_N;
                ParamByName('EQ_STATE').AsInteger := cbState.Value;
                ParamByName('NOTICE').AsString := mmoNotice.Lines.Text;
                if cbEncode.Text <> '' then
                  ParamByName('TEXT_ENCODE').AsString := cbEncode.Text
                else
...
procedure TDVBEqEditForm.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
