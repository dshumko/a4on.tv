...
function RequestResultModify(const RR_ID: Integer; const RT_ID: Integer; const RT_NAME: string): Boolean;
...
begin
  ...
  with TRequestResult.Create(Application) do
    try
      ...
      if dsResult.State in [dsInsert, dsEdit] then
        dsResult.Cancel;
    finally
      Free;
    end;
...
procedure TRequestResult.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Shift = [ssCtrl]) and (Ord(Key) = VK_RETURN) then
    frmOkbbOkClick(Sender);
end;
