...
var
  NodesFilterForm: TNodesFilterForm;

implementation

uses DM, MAIN, AtrCommon, NodesForma;

{$R *.dfm}

procedure TNodesFilterForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  dsStreets.Close;
  dsHomes.Close;
  dsArea.Close;
...
procedure TNodesFilterForm.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Shift = [ssCtrl]) and (Ord(Key) = VK_RETURN) then
    bbOkClick(Sender);
end;

procedure TNodesFilterForm.FormKeyPress(Sender: TObject; var Key: Char);
var
  go: boolean;
begin
  if (Key = #13) then
...
procedure TNodesFilterForm.FormShow(Sender: TObject);
begin
  dsStreets.Open;
  dsHomes.Open;
  dsArea.Open;
...
procedure TNodesFilterForm.SaveFilter(const filename: string);
var
  s: string;
begin
...
procedure TNodesFilterForm.SpeedButton2Click(Sender: TObject);
begin
...
procedure TNodesFilterForm.actClearExecute(Sender: TObject);
begin
  if not Assigned(NodesForm) then
    exit;

  with NodesForm.dsFilter do
  begin
    DisableControls;
    Close;
    Open;
...
procedure TNodesFilterForm.bbOkClick(Sender: TObject);
begin
  if srcFilter.DataSet.State in [dsEdit, dsInsert] then
  begin
    srcFilter.DataSet.post;
  ...
  if chkDefaultFilter.Checked then
    SaveFilter('nodes_default');
  ModalResult := mrOk;
end;

procedure TNodesFilterForm.btnANDClick(Sender: TObject);
begin
  srcFilter.DataSet.Append;
  srcFilter.DataSet['next_condition'] := 1;
end;

procedure TNodesFilterForm.btnORClick(Sender: TObject);
begin
  srcFilter.DataSet.Append;
  srcFilter.DataSet['next_condition'] := 0;
end;

procedure TNodesFilterForm.checkAdressSign(Sender: TObject);
begin
